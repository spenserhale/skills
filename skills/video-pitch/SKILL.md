...
5. **Record the demo.** Real product, real actions, sized to the demo beat's duration from `timing.json`. Web app: write a steps file and run `node scripts/record_web_demo.mjs steps.json --out <out>/work`. CLI: build a self-playing terminal page with `scripts/build_terminal_page.py`, then record it the same way. Desktop or mobile: ask the user for a recording and use it as the clip. Recipes, selectors, login handling, and legibility rules: `references/record.md`. Done when `marks.json` shows the flow inside the recording and one extracted frame is legible at 1080p.
6. **Cards.** Render the hook and closing cards (and any statement card) with `scripts/render_card.py`, using the product's colours and font from the config or its stylesheet. Cards carry the claim in a few words; they never repeat the narration verbatim.
...
| Manifest schema, fitting clips, captions, music, poster, vertical output, polish with hyperframes | `references/render.md` |
